    ...
    def to_dict(self):
        ...
        if self.hire_date:
            try:
                ...
                hd = ''
        return {'id':self.id,'name':self.name,'phone':self.phone,'staff_type':self.staff_type,'daily_wage':self.daily_wage,'monthly_salary':self.monthly_salary,'project':self.project,'position':self.position,'id_card':self.id_card,'id_photo':self.id_photo,'cert_photo':self.cert_photo,'hire_date':hd,'status':self.status or 'active','remark':self.remark}
...
class PaymentRecord(db.Model):
    ...
    def to_dict(self):
        return {
            ...
            'is_invoiced': self.is_invoiced or False,
            'remark': self.remark or '',
...
class Expense(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    expense_type = db.Column(db.String(20), default='other')  # purchase/salary/daily/other
    category = db.Column(db.String(50), default='')  # 分类：物料采购/房租/水电/办公用品/差旅/其他
    title = db.Column(db.String(200), default='')
    amount = db.Column(db.Float, default=0.0)
    expense_date = db.Column(db.Date, index=True)
    related_type = db.Column(db.String(30), default='')  # material/stock_log/record
    related_id = db.Column(db.Integer)
    related_no = db.Column(db.String(100), default='')
    supplier = db.Column(db.String(100), default='')  # 供应商/收款方
    payment_method = db.Column(db.String(20), default='cash')  # cash/bank/wechat/alipay/other
    remark = db.Column(db.Text, default='')
    created_by = db.Column(db.String(100), default='')
    created_at = db.Column(db.DateTime, default=datetime.now)
