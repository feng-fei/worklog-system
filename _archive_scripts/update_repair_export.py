...
# -*- coding: utf-8 -*-
"""
Complete replacement for repair_export_routes.py with:
1. _num_to_chinese function added
2. fill_template updated with new field mappings
"""

file_path = '/app/app/routes/repair_export_routes.py'

new_content = r'''import io
import openpyxl
...
def _num_to_chinese(num):
    ...
    if integer_part == 0:
        ...
        for i, d in enumerate(s):
            ...
            d_int = int(d)
            if d_int != 0:
                result += digits[d_int] + units[unit_pos]
            else:
...
def fill_template(ws, record):
    ...
    safe_write_cell(ws, 4, 1, f'维修服务单                                    工单编号：{order_no}')

    # === 受理时间 / 维修时间 (Row 5) ===
    safe_write_cell(ws, 5, 2, record.work_date.strftime('%Y年%m月%d日') if record.work_date else '')

    # 到达时间 / 完工时间 - 写入 Row 5 的维修时间位置
    arrival = record.arrival_time or ''
    completion = record.completion_time or ''
    if arrival or completion:
        time_str = f"{arrival} - {completion}" if arrival and completion else (arrival or completion)
        safe_write_cell(ws, 5, 6, time_str)
    else:
        # 使用原有的维修时间逻辑
        time_range = ''
    ...
    # 项目名称处写入服务类别
    safe_write_cell(ws, 6, 6, record.service_category or '售后维修服务')
    ...
    # 保修情况
    warranty_map = {'在保': '●在保  ○过保  ○无保', '过保': '○在保  ●过保  ○无保', '无保': '○在保  ○过保  ●无保'}
    safe_write_cell(ws, 8, 6, warranty_map.get(record.warranty_status, '○在保  ○过保  ○无保'))

    # === 故障状况 (Row 9-13 merged) ===
    # 设备信息拼接到故障描述
    device_info = ''
    if record.device_brand or record.device_model or record.device_sn:
        parts = []
        if record.device_brand:
            parts.append(f'品牌：{record.device_brand}')
        if record.device_model:
            parts.append(f'型号：{record.device_model}')
        if record.device_sn:
            parts.append(f'序列号：{record.device_sn}')
        device_info = '设备信息：' + '，'.join(parts) + '\n'

    fault_desc = record.fault_description or record.work_content or ''
    if device_info:
        safe_write_cell(ws, 9, 1, f'{device_info}故障状况：\n{fault_desc}')
    else:
        safe_write_cell(ws, 9, 1, fault_desc)
    ...
    # 大写金额写入 Row 34
    tax_label = '含税' if record.tax_type == 'tax' else '未税'
    safe_write_cell(ws, 34, 1, f'总计（{tax_label}）：{total}元\n大写：{_num_to_chinese(total)}')

    # === 付款方式 (Row 35) ===
    payment_map = {'cash': '现金', 'wechat': '微信', 'alipay': '支付宝', 'transfer': '转账', 'unpaid': '未付款', 'paid': '已付款', 'partial': '部分付款', 'monthly': '月结'}
    ...
    # === 客户意见 (Row 37) ===
    feedback = record.customer_feedback or ''
    satisfaction = record.satisfaction or ''
    feedback_text = '客户意见：\n' + feedback if feedback else '客户意见：\n'
    if satisfaction:
        feedback_text += f'\n满意度：{satisfaction}'
    safe_write_cell(ws, 37, 1, feedback_text)
    ...
    safe_write_cell(ws, 43, 1, f'  {sat_map.get(record.satisfaction, "○满意 ○比较满意 ○一般 ○不满意")}                               客户签字：')
    ...
    # === 保修条款 (Row 49 之后) ===
    warranty_days = record.warranty_days or 30
    safe_write_cell(ws, 49, 1, f'保修条款：本维修服务自维修完成之日起提供{warranty_days}天保修期，保修范围仅限本次维修更换的配件及维修工艺。')
