...
(function() {
  ...
  function fetchReceipts() {
    return new Promise(function(resolve) {
      if (typeof window.apiFetch === 'function') {
        window.apiFetch('/receipts?limit=1000', function(res) {
          resolve((res && res.data) || (res && res.list) || []);
        }, function() { resolve([]); });
      } else {
        resolve([]);
      }
  ...
  function fetchExpenses() {
    return new Promise(function(resolve) {
      if (typeof window.apiFetch === 'function') {
        window.apiFetch('/expenses?limit=1000', function(res) {
          resolve((res && res.data) || (res && res.list) || []);
        }, function() { resolve([]); });
      } else {
        resolve([]);
  ...
  function fetchSalary() {
    return new Promise(function(resolve) {
      if (typeof window.apiFetch === 'function') {
        window.apiFetch('/salary?limit=1000', function(res) {
          resolve((res && res.data) || (res && res.list) || []);
        }, function() { resolve([]); });
      } else {
        resolve([]);
  ...
  function mergeTransactions(receipts, expenses, salary) {
    ...
    receipts.forEach(function(r) {
      transactions.push({
        ...
        typeLabel: '收款',
        typeClass: 'success',
        date: r.date || r.paymentDate || r.createdAt,
        amount: Number(r.amount || r.total || 0),
        title: r.customerName || r.title || '收款',
        desc: r.remark || r.description || '',
    ...
    expenses.forEach(function(e) {
      transactions.push({
        ...
        title: e.category || e.title || '支出',
        desc: e.remark || e.description || '',
    ...
    salary.forEach(function(s) {
      transactions.push({
        ...
        typeLabel: '工资',
        typeClass: 'warning',
        date: s.date || s.payDate || s.createdAt,
        amount: -Number(s.amount || s.total || 0),
        title: s.employeeName || s.title || '工资',
        desc: s.remark || s.description || '',
...
(function() {
  ...
  function fetchProjectReceipts(projectId) {
    return new Promise(function(resolve) {
      if (typeof window.apiFetch === 'function') {
        window.apiFetch('/receipts?projectId=' + projectId, function(res) {
          resolve((res && res.data) || (res && res.list) || []);
        }, function() { resolve([]); });
      } else {
        resolve([]);
  ...
  function fetchProjectExpenses(projectId) {
    return new Promise(function(resolve) {
      if (typeof window.apiFetch === 'function') {
        window.apiFetch('/expenses?projectId=' + projectId, function(res) {
          resolve((res && res.data) || (res && res.list) || []);
        }, function() { resolve([]); });
      } else {
        resolve([]);
  ...
  function fetchProjectSalary(projectId) {
    return new Promise(function(resolve) {
      if (typeof window.apiFetch === 'function') {
        window.apiFetch('/salary?projectId=' + projectId, function(res) {
          resolve((res && res.data) || (res && res.list) || []);
        }, function() { resolve([]); });
      } else {
        resolve([]);
