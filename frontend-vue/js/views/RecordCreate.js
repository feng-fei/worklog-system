const RecordCreate = {
    ...
    <div class="record-create">
      <div class="page-card">
        ...
        <el-form :model="form" label-width="100px" ref="formRef">
          <el-row :gutter="24">
            ...
            <el-col :span="12">
              <el-form-item label="客户" prop="customer_id">
                <el-select v-model="form.customer_id" filterable placeholder="请选择客户">
          ...
          <el-form-item v-if="form.record_type === 'repair'" label="故障描述">
          ...
          <div class="dynamic-section" v-if="form.record_type === 'repair'">
            <div class="section-header">
              <span>使用物料 / 设备</span>
  ...
  setup() {
    ...
    const loadOptions = async () => {
      try {
        const [custRes, staffRes, matRes] = await Promise.all([
          apiService.getCustomers({ per_page: 1000 }),
          apiService.getStaffs({ per_page: 1000 }),
          apiService.getMaterials({ per_page: 1000 })
        ]);

        customers.value = (custRes && Array.isArray(custRes.records)) ? custRes.records : [];
        staffList.value = (staffRes && Array.isArray(staffRes.records)) ? staffRes.records : [];
        materials.value = (matRes && Array.isArray(matRes.records)) ? matRes.records : [];

        try {
          const tempRes = await apiService.getWorkTemplates({ per_page: 100 });
          templates.value = (tempRes && tempRes.records) ? tempRes.records : [];
        } catch (e) {
    ...
    const totalFee = computed(() => {
      if (!Array.isArray(form.fee_items)) return 0;
    ...
    const applyTemplate = async (templateId) => {
      ...
      try {
        ...
        if (res) {
          if (res.fault_description) {
            form.fault_description = res.fault_description;
          }
          if (res.construction_content) {
            form.construction_content = res.construction_content;
          }
          if (res.service_address) {
            form.service_address = res.service_address;
          }
